...
"""
Script to generate Openium version numbers
...
def check_existing_version(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        if "OPENIUM" in file.read():
            raise ValueError("file already contains openium versioning")
...
def get_version_parts(tree, platform_tree):
    ...
    version_paths = {
        "OPENIUM_MAJOR": tree / "version.txt",
        "OPENIUM_MINOR": tree / "chromium_version.txt",
        "OPENIUM_PATCH": tree / "revision.txt",
    }

    if platform_tree is not None:
        version_paths["OPENIUM_PLATFORM"] = platform_tree / "revision.txt"

    version_parts = {}
    for name, path in version_paths.items():
        delta = 0 if name != "OPENIUM_MINOR" else -CHROME_VERSION_BASE
