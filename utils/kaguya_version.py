...
def check_existing_version(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        if "KAGUYA" in file.read():
            raise ValueError("file already contains kaguya versioning")
...
def get_version_parts(tree, platform_tree):
    ...
    version_paths = {
        "KAGUYA_MAJOR": tree / "version.txt",
        "KAGUYA_MINOR": tree / "chromium_version.txt",
        "KAGUYA_PATCH": tree / "revision.txt",
    }

    if platform_tree is not None:
        version_paths["KAGUYA_PLATFORM"] = platform_tree / "revision.txt"

    version_parts = {}
    for name, path in version_paths.items():
        delta = 0 if name != "KAGUYA_MINOR" else -CHROME_VERSION_BASE
